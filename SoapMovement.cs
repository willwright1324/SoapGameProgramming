...
public class SoapMovement : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if (!pause && sCam.done == true) {
            if (!riding) {
                if (Input.GetKey("a")) {
                    rb.AddForce(-moveDir.transform.right * speed, ForceMode.VelocityChange);
                }
                if (Input.GetKey("d")) {
                    rb.AddForce(moveDir.transform.right * speed, ForceMode.VelocityChange);
                }
                if (Input.GetKey("w")) {
                    rb.AddForce(moveDir.transform.forward * speed, ForceMode.VelocityChange);
                }
                if (Input.GetKey("s")) {
                    rb.AddForce(-moveDir.transform.forward * speed, ForceMode.VelocityChange);
                }
    ...
    private void LateUpdate() {
        ...
        if (jPos1B.y < jPos2B.y) {
            jumpTrigger.transform.position = new Vector3(transform.position.x, jPos1B.y - 0.5f, transform.position.z);
        }
        else {
            jumpTrigger.transform.position = new Vector3(transform.position.x, jPos2B.y - 0.5f, transform.position.z);
        }
    ...
    //Boosts player with suds, absorbs suds, collects points, and ends game with totaled scores/time
    ...
    private void OnTriggerStay(Collider other) {
        ...
        if (other.gameObject.tag == "Suds" && !boost) {
            ...
            if ((Input.GetKey("w") || Input.GetKey("a") || Input.GetKey("s") || Input.GetKey("d")/*|| Input.GetKey(KeyCode.Space)*/) && canBoost && Vector3.Distance(transform.position, other.transform.position) < 2) {
                boost = true;
